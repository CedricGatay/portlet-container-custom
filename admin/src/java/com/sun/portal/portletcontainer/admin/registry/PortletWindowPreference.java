...
/**
 * PortletWindowPreference represents the PortletWindowPreference Element
...
public class PortletWindowPreference extends AbstractPortletRegistryElement {
    ...
    protected void populateValues(Element portletWindowPrefTag) {
        ...
        Map portletWindowPreferenceAttributes = XMLDocumentHelper.createAttributeTable(portletWindowPrefTag);
        setName((String)portletWindowPreferenceAttributes.get(NAME_KEY));
        setPortletName((String)portletWindowPreferenceAttributes.get(PORTLET_NAME_KEY));
        setUserName((String)portletWindowPreferenceAttributes.get(USER_NAME_KEY));
        super.populateValues(portletWindowPrefTag);
    }
    
    protected void populateValues(PortletWindowPreferenceRegistryModel portletWindowPreferenceRegistryModel) {

		setName(portletWindowPreferenceRegistryModel.getName());
        setPortletName(portletWindowPreferenceRegistryModel.getPortletName());
		setUserName(portletWindowPreferenceRegistryModel.getUserName());
        super.populateValues(portletWindowPreferenceRegistryModel.getProperties());
    }

    protected void create(Document document, Element rootTag) {
        // Create PortletWindow tag, addPreference attributes and append it to the document
        Element portletWindowPrefTag = XMLDocumentHelper.createElement(document, PORTLET_WINDOW_PREFERENCE_TAG);
	...
	public int compareTo(Object portletWindowPreference) {
		return this.getName().compareTo(((PortletWindowPreference)portletWindowPreference).getName());
	}
}
