/*
 * CDDL HEADER START
 * The contents of this file are subject to the terms
 * of the Common Development and Distribution License
 * (the License). You may not use this file except in
 * compliance with the License.
 *
 * You can obtain a copy of the License at
 * http://www.sun.com/cddl/cddl.html and legal/CDDLv1.0.txt
 * See the License for the specific language governing
 * permission and limitations under the License.
 *
 * When distributing Covered Code, include this CDDL
 * Header Notice in each file and include the License file
 * at legal/CDDLv1.0.txt.
 * If applicable, add the following below the CDDL Header,
 * with the fields enclosed by brackets [] replaced by
 * your own identifying information:
 * "Portions Copyrighted [year] [name of copyright owner]"
 *
 * Copyright 2009 Sun Microsystems Inc. All Rights Reserved
 * CDDL HEADER END
 */

package com.sun.portal.container.service.policy.impl;

import com.sun.portal.container.service.policy.ContainerEventPolicy;
import com.sun.portal.container.service.policy.DistributionType;

/**
 * ContainerEventPolicyImpl provides a default implementation of the ContainerEventPolicy
 *
 */
public class ContainerEventPolicyImpl implements ContainerEventPolicy {

    private DistributionType distributionType;
    private int maxGenerationOfEvents;
    private boolean enabled;
    
    public ContainerEventPolicyImpl() {
        this.distributionType = DistributionType.ALL_PORTLETS_ON_PAGE;
        this.maxGenerationOfEvents = DEFAULT_MAX_EVENT_GENERATION;
        this.enabled = true;
    }

    public ContainerEventPolicyImpl(DistributionType distributionType, 
            int maxGenerationOfEvents, boolean enabled) {
        this.distributionType = distributionType;
        this.maxGenerationOfEvents = maxGenerationOfEvents;
        this.enabled = enabled;
    }

    public DistributionType getDistributionType() {
        return this.distributionType;
    }

    public int getMaxGenerationOfEvents() {
        return this.maxGenerationOfEvents;
    }

    public boolean isEnabled(String eventName) {
        return this.enabled;
    }
}
