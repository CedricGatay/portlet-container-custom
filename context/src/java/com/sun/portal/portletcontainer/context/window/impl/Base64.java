...
class Base64 {
    ...
    /**
     * Convert the String into an array of bytes then send the bytes
     * through the
    ...
    public static byte[] encode(byte[] theBytes) {
        ...
        while ((i + 2) < theBytes.length) {
            ...
            encodedBytes[++place] = theMap[((byte1 << 4) & 0x30) + ((byte2 >>> 4) & 0xf)];
            encodedBytes[++place] = theMap[((byte2 << 2) & 0x3c) + ((byte3 >>> 6) & 0x3)];
